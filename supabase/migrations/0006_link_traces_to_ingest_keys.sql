alter table ledge_private.ledge_ingest_keys
    add constraint ledge_ingest_keys_app_id_id_key unique (app_id, id);

alter table ledge_private.ledge_traces
    add column ingest_key_id uuid not null,
    add constraint ledge_traces_ingest_key_fk
        foreign key (app_id, ingest_key_id)
        references ledge_private.ledge_ingest_keys (app_id, id)
        on delete no action
